/* backtracking from both sides with meet-in-the-middle
   ...
   use move ordering based on left-pruning
   iterative widening */
...
int compm(const void *A,const void *B) {
	const move_t *a=A,*b=B;
...
int btrfront(int at,int w) {
	int i,j,k,l,left,mv,m;
	move_t moves[MAXX*MAXY];
	if(at==meet) {
	...
	for(i=0;i<=x-pcx[at];i++) for(j=0;j<=y-pcy[at];j++) {
		for(k=0;k<pcx[at];k++) for(l=0;l<pcy[at];l++) if(pieces[at][k][l]) INC(board,i+k,j+l);
		left=calcprune3(at+1);
		if(left>prefix[at+1]) { 	
			pathx[pid[at+1]]=i; pathy[pid[at+1]]=j;
			info(at+1); goto feil;
		}
		moves[mv].x=i;
		moves[mv].y=j;
	...
	qsort(moves,mv,sizeof(move_t),compm);
	for(m=0;m<mv && m<w;m++) {
		i=moves[m].x;
		j=moves[m].y;
		for(k=0;k<pcx[at];k++) for(l=0;l<pcy[at];l++) if(pieces[at][k][l]) INC(board,i+k,j+l);
		pathx[pid[at]]=i; pathy[pid[at]]=j;
		if(btrfront(at+1,w)) return 1;
		for(k=0;k<pcx[at];k++) for(l=0;l<pcy[at];l++) if(pieces[at][k][l]) DEC(board,i+k,j+l);
	}
	return 0;
}
