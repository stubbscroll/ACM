...
/* same as astar2.c, but with backtracking+pruning instead of memoization */
...
void hbtr(int col,int at,ll mask,int cur) {
	int i;
	if(cur>=best) return;
	while(at<wn && wcol[at]!=col) at++;
	if(at==wn) { best=cur; return; }
//	printf("at col %d at %d mask %I64d\n",col,at,mask);printf("collen %d hdp[col] %I64d\n",collen[col],(ll)hdp[col]);
	for(i=0;i<pnsub[at];i++) if(!(mask&psubset[at][i])) hbtr(col,at+1,mask|psubset[at][i],cur+hsearch(at,i));
}
...
void solve() {
	...
	printf("memory used in patterns: %I64d\n",patmem);
