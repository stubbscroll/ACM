...
/* bfs v3: halve memory usage when coordinates are <16,
   + an additional halving of the memory usage by storing backwards move
   instead of entire parent state */
...
void initbfs() {
	...
	pushed=0;
	rpn=0;
	for(i=0;i<wn;i++) hsize+=wlen[i]*2/(half+1);
	dsize=8/(half+1);
