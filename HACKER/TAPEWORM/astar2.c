/* TODO ideas:
   - graceful handling of out-of-memory in pattern database bfs, just use
     current state space and use another heuristic if we don't hit database
   - shorten move/backmove: worm id + bit (head/tail) + dir 0-5. 4 bits for
     worm id, 1 bit for head/tail, 3 bits for dir (0-5)
   - find a way to speed up h() on puzzles with many same-colour worms
     (memoization?)
*/

/* same as astar.c, but with more compact state representation: store
   start coordinate of each worm, plus dirs (0-5) encoded as a base 6 number,
   3 dirs per byte (6^3=216<256) */
...
#define MAXCOL 10 /* highest worm colour */
...
int level;
int collen[MAXCOL];                /* total worm length by colour */
int numcol[MAXCOL];                /* number of worms of this colour */
...
void parselevel() {
	...
	for(i=0;i<MAXCOL;i++) collen[i]=numcol[i]=0;
	...
	while(1) {
		...
		id=*(p++)-48;
		wcol[wn]=id;
		if(id>=MAXCOL) error("increase MAXCOL and recompile");
		wlen[wn]=0;
		while(1) {
			if(*p!='_') break;
			p++;
...
//	printf("decoded:");for(i=0;i<hsize;i++)printf(" %d",cur[i]);printf(" =>");
	if(half) px[0]=cur[p]&15,py[0]=cur[p++]>>4;
	else px[0]=cur[p++],py[0]=cur[p++];
	for(m=i=1;i<l;i++) {
		d=cur[p]/m%6;
		fixd(px[i-1]);
		px[i]=px[i-1]+dx[d];
...
//	printf("encoded:");for(i=0;i<l;i++) printf(" (%d %d)",px[i],py[i]);printf(" =>");
	if(half) cur[p++]=px[0]+(py[0]<<4);
	else cur[p++]=px[0],cur[p++]=py[0];
	for(i=m=1;i<l;i++) {
		if(m==1) cur[p]=0;
		fixd(px[i-1]);
		for(d=0;d<6;d++) if(px[i-1]+dx[d]==px[i] && py[i-1]+dy[d]==py[i]) break;
//		printf("from (%d %d) to (%d %d) d %d\n",px[i-1],py[i-1],px[i],py[i],d);
		if(d==6) error("error in encode1");
		cur[p]+=d*m;
		m*=6; if(m==216) m=1,p++;
	}
...
#define MAXSAME 3
uchar hdp[1<<MAXMEMO];
int hq[(1<<MAXMEMO)*sizeof(int)],hqn;

int hbtr(int col,int at,ll mask) {
	int i,r=1000000,cur;
	while(at<wn && wcol[at]!=col) at++;
	if(at==wn) return 0;
//	printf("at col %d at %d mask %I64d\n",col,at,mask);printf("collen %d hdp[col] %I64d\n",collen[col],(ll)hdp[col]);
	if(numcol[col]>=MAXSAME && collen[col]<=MAXMEMO && hdp[mask]<255) return hdp[mask];
	for(i=0;i<pnsub[at];i++) if(!(mask&psubset[at][i])) {
		cur=hbtr(col,at+1,mask|psubset[at][i])+hsearch(at,i);
		if(r>cur) r=cur;
	}
	if(numcol[col]>=MAXSAME && collen[col]<=MAXMEMO) hq[hqn++]=mask,hdp[mask]=r;
	return r;
}
...
int h() {
	...
	for(col=0;col<MAXCOL;col++) if(collen[col]) {
		z=hbtr(col,0,0);
		if(numcol[col]>=MAXSAME && collen[col]<=MAXMEMO) while(hqn) hdp[hq[--hqn]]=255;
		r+=z;
	}
	return r;
}
...
void printheap() {
	...
	for(i=1;i<heapn;i++) {
		...
		for(j=0;j<hsize;j++) printf("%d ",heapid[i*hsize+j]);
...
int astar() {
	...
	for(hsize=i=0;i<wn;i++) hsize+=2-half+(1+wlen[i])/3;
